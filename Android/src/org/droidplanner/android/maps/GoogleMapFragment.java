...
public class GoogleMapFragment extends SupportMapFragment implements DPMap,
    ...
    public double rcv_lat = 0;
    public double rcv_lon = 0;
    ...
    private final LocationCallback locationCb = new LocationCallback() {
        ...
        public void onLocationResult(LocationResult result) {
            ...
            //double latDrone = SignletonLatLng.getInstance(getContext()).getLat();
            //double lngDrone = SignletonLatLng.getInstance(getContext()).getLon();
            ...
            if (userMarker == null) {
                ...
                getMapAsync(new OnMapReadyCallback() {
                    ...
                    public void onMapReady(GoogleMap googleMap) {
                        ...
                        //userMarkerDrone = googleMap.addMarker(options1);
                    }
                });
            } else {
            ...
            if (mPanMode.get() == AutoPanMode.USER) {
                ...
                //location.setLatitude(latDrone);
                //location.setLongitude(lngDrone);
                //updateCamera(MapUtils.locationToCoord(location), (int) getMap().getCameraPosition().zoom);
            }
            if (mPanMode.get() == AutoPanMode.DRONE) {
                Timber.d("User location changed.");
    ...
    public GoogleApiClientTask mGoToCustomLocationTask = new GoogleApiClientTask() {
        @Override
        public void doRun() {

            final Location myLocation = LocationServices.FusedLocationApi.getLastLocation(getGoogleApiClient());

            myLocation.setLatitude(SignletonLatLng.getInstance(getContext()).getLat());
            myLocation.setLongitude(SignletonLatLng.getInstance(getContext()).getLon());
            ...
            for (int i = 0; i < 3; i++) {
                getMapAsync(new OnMapReadyCallback() {
                    @Override
                    public void onMapReady(GoogleMap googleMap) {
                        double[] dummyLat = {22.821667, 22.822779, 22.823997};
                        double[] dummyLon = {89.551869, 89.552879, 89.553899};

                        final MarkerOptions testOption = new MarkerOptions()
                                .position(new LatLng(dummyLat[count], dummyLon[count]))
                                .anchor(0.5f, 0.5f)
                                .title("Drone")
                                .snippet("")
                                .draggable(false)
                        ...
                        userMarkerDrone = googleMap.addMarker(testOption);
                        userMarkerDrone.setPosition(new LatLng(dummyLat[count], dummyLon[count]));
                        myLocation.setLatitude(dummyLat[count]);
                        myLocation.setLongitude(dummyLon[count]);
                        updateCamera(MapUtils.locationToCoord(myLocation), (int) getMap().getCameraPosition().zoom);
    ...
    private Marker userMarkerDrone;
    ...
    public void onStart() {
        ...
        // ferdib - al - islam << handler can be implemented here
        mapHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                if (userMarkerDrone != null) {
                    userMarkerDrone.setVisible(false);
                }
                mapHandler.removeCallbacks(this);
                mGApiClientMgr.addTask(showLocation);
                mapHandler.postDelayed(this, 1000L);
            }
        }, 1000L);
    }

    GoogleApiClientTask showLocation = new GoogleApiClientTask() {
        @Override
        protected void doRun() {
            final Location loc = LocationServices.FusedLocationApi.getLastLocation(getGoogleApiClient());
            final double tempLat = SignletonLatLng.getInstance(getContext()).getLat();
            final double tempLon = SignletonLatLng.getInstance(getContext()).getLon();
            final float droneRotation = SignletonGyroscopeUpdate.getInstance(getContext()).getYaw();
            ...
            getMapAsync(new OnMapReadyCallback() {
                ...
                public void onMapReady(GoogleMap googleMap) {
                    final MarkerOptions markerOptions = new MarkerOptions()
                            ...
                            .flat(true)
                            .visible(true)
                            .icon(BitmapDescriptorFactory.fromResource(R.drawable.redcopter));
                    userMarkerDrone = googleMap.addMarker(markerOptions);
                    userMarkerDrone.setPosition(new LatLng(tempLat, tempLon));
                    loc.setLatitude(tempLat);
                    loc.setLongitude(tempLon);
                    //updateCamera(MapUtils.locationToCoord(loc), (int) getMap().getCameraPosition().zoom);
                    updateCamera(MapUtils.locationToCoord(loc), GO_TO_MY_LOCATION_ZOOM);
                }
            });
        }
    };
    ...
    public void goToDroneLocation() {
        ...
        (getActivity()).runOnUiThread(new Runnable() {
            public void run() {
                if (!mGApiClientMgr.addTask(mGoToCustomLocationTask)) {
                    Timber.e("Unable to add google api client task.");
                }
    ...
    public void mGoToCustomLocationTask() {
